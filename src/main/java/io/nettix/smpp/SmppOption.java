 ...
 * @author sanha
 */
public enum SmppOption
{
  /**
   * The dest_addr_subunit parameter is used to route messages when received by
   * a mobile station, for example to a smart card in the mobile station or to
   * an external device connected to the mobile station.
   */
  DEST_ADDR_SUBUNIT(0x0005),

  /**
   * The dest_network_type parameter is used to indicate a network type
   * associated with the destination address of a message. In the case that the
   * receiving system (e.g. SMSC) does not support the indicated network type,
   ...
   * bearer dependent. In the case that the receiving system (e.g. SMSC) does
   * not support the indicated telematic interworking, it may treat this a
   ...
   * PLMN. The indication can be an icon on the MS screen or other MMI
   * indication. The ms_msg_wait_facilities can also specify the type of message
  ...
  /**
   * The sar_msg_ref_num parameter is used to indicate the reference number for
   * a particular concatenated short message.
   */
   ...
   * a particular short message within the concatenated short message.
   */
   ...
   * CallBackNumber at the mobile station.If present, the callback_num parameter
   ...
   * In TDMA networks, it is possible to send and receive multiple callback
   * numbers to/from TDMA mobile stations.
   */
  CALLBACK_NUM(0x0381),
   ...
   * message.. If the dpf_result parameter is not included in the data_sm_resp
   * PDU, the ESME should assume that DPF is not set. Currently this parameter
   * is only applicable for the Transaction message mode.
   */
  DPF_RESULT(0x0420),

  /**
   * An ESME may use the set_dpf parameter to request the setting of a delivery
   * pending flag (DPF) for certain delivery failure scenarios, such as - MS is
   * unavailable for message delivery (as indicated by the HLR) The SMSC should
   * respond to such a request with an alert_notification PDU when it detects
   * that the destination MS has become available. The delivery failure
   * scenarios under which DPF is set is SMSC implementation and network
   * implementation specific. If a delivery pending flag is set by the SMSC or
   * network (e.g. HLR), then the SMSC should indicate this to the ESME in the
   ...
   * operation to indicate the availability state of the MS to the ESME. If the
   * SMSC does not include the parameter in the alert_notification operation,
   * the ESME should assume that the MS is in an “available” state.
   */
   ...
   * to indicate the outcome of the message delivery attempt (only applicable
   * for transaction message mode). If a delivery failure due to a network error
   * is indicated, the ESME may check the network_error_code parameter (if
   * present) for the actual network error code. The delivery_failure_reason
   * parameter is not included if the delivery attempt was successful
   */
   ...
   * and data_sm PDUs to indicate to the ESME the final message state for an
   ...
   * indicates and controls the MS user’s reply method to an SMS delivery
   ...
   * Interactive Teleservice as defined by the Korean PCS carriers [KORITS]. It
   * contains control information for the interactive session between an MS and
   * an ESME.
   */
